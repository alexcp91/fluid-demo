...
interface ColorPickerPopoverProps extends ColorPickerProps {
   ...
   *  Second arg is Base UI event details (supports `.cancel()`). */
  onOpenChange?: (
    open: boolean,
    eventDetails?: Popover.Root.ChangeEventDetails
  ) => void;
  /**
   * Focus management when the panel opens. Pass `false` to leave focus where
   * it is — needed when the picker sits on a contentEditable format toolbar
   * and must not collapse the text selection.
   */
  initialFocus?: Popover.Popup.Props["initialFocus"];
  /** Focus management when the panel closes. Pass `false` to skip restore. */
  finalFocus?: Popover.Popup.Props["finalFocus"];
  /**
   * Prevent mousedown inside the panel (except text inputs) from collapsing
   * an outside text selection — for inline format toolbars.
   */
...
const ColorPickerPopover = forwardRef<HTMLDivElement, ColorPickerPopoverProps>(
  (
    ...
    const handleOpenChange = useCallback(
      (next: boolean, eventDetails: Popover.Root.ChangeEventDetails) => {
        onOpenChange?.(next, eventDetails);
        // Consumer may cancel (e.g. ignore focus-out while applying a color).
              ...
              >
                <Popover.Popup
                  ...
                  onMouseDown={(e) => {
                    ...
                    // Keep an outside contentEditable selection alive while
                    // scrubbing the saturation square / swatches / sliders.
                    e.preventDefault();
                  }}
                >
